...
🛠️ Tech Stack
The full tech‑stack is documented in 
TECH_STACK.md
, but the core components are:
...
📹 Usage
Open the AR Interface (http://localhost:3000) in a browser that supports WebRTC.
Grant camera permissions.
The app streams video, detects faces, and shows real‑time bounding boxes.
Detected face embeddings are sent to the back‑end, stored in MongoDB, and can be queried for recognition.
Speech captured via the Web Speech API is transcribed and fed to the AI summarizer, which returns a concise text summary displayed to the user.
🧪 Testing
